...
for L in range(2, 8):
    ...
    trL = vec.lin_to_mat(lambda X : qu.p_tr(X, dims, L-1), (N, m), compact=(True, False))
    tr  = vec.mat_to_vec(np.eye(N), compact=True).T
    ikr_tr1 = vec.lin_to_mat(lambda X : qu.i_kr(qu.p_tr(X, dims, 0), dims, 0), (N, N), compact=(True, False))
