...
for n in range(2, 17):
    ...
    c = np.vstack((np.array([[1.]]), np.zeros((vn, 1))))
    ...
    # tr[X] = 1
    A = np.hstack((np.array([[0.]]), vec.mat_to_vec(np.eye(n, dtype=np.complex128), compact=True).T))
    b = np.array([[1.]])

    G1 = np.hstack((np.array([[1.]]), np.zeros((1, vn))))
