...
for n in range(2, 17):
    ...
    c = np.vstack((np.array([[1.0], [1.0]]), np.zeros((vn, 1))))
    ...
    G1 = np.hstack((np.array([[1.0, 0.0]]), np.zeros((1, vn))))
    G2 = np.hstack((np.zeros((2 * N * N, 2)), VV))
    G3 = np.hstack((np.zeros((2 * N * N, 2)), ikr_trB @ VV))
    G4 = np.hstack((np.array([[0.0, 1.0]]), np.zeros((1, vn))))
    G5 = np.hstack((np.array([[0.0, 0.0]]), np.zeros((1, vn))))
