...
for n in range(2, 12):
    ...
    vN = vec.vec_dim(N, compact=True, iscomplex=True)
    VV = vec.lin_to_mat(
        lambda X: V @ X @ V.conj().T, (n, n * n), iscomplex=True, compact=(True, False)
    )
    trE = vec.lin_to_mat(
        lambda X: qu.p_tr(X, (n, n), 0), (N, n), compact=(False, False), iscomplex=True
    )

    G1 = np.hstack((np.array([[1.0, 0.0]]), np.zeros((1, vn))))  # t_cond
    G2 = np.hstack((np.zeros((2 * N * N, 2)), VV))  # X_cond
    G3 = np.hstack((np.array([[0.0, 1.0]]), np.zeros((1, vn))))  # t_entr
    G4 = np.hstack((np.array([[0.0, 0.0]]), np.zeros((1, vn))))  # y_entr
    G5 = np.hstack((np.zeros((2 * n * n, 2)), trE @ VV))  # X_entr
    G6 = np.hstack((np.zeros((2 * n * n, 2)), vec.eye(n, iscomplex=True).T))  # X_psd

    h1 = np.array([[0.0]])  # t_cond
    h2 = np.zeros((2 * N * N, 1))  # X_cond
    h3 = np.array([[0.0]])  # t_entr
    h4 = np.array([[1.0]])  # y_entr
